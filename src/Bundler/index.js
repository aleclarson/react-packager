...
class Bundler {
  ...
  constructor(options) {
    ...
    let mtime;
    try {
      ({mtime} = fs.sync.stat(opts.transformModulePath));
  ...
  _buildBundle({
    ...
    return resolutionResponse.then(response => {
...
function verifyRootExists(root) {
  ...
  assert(fs.sync.stat(root).isDirectory(), 'Root has to be a valid directory');
}
